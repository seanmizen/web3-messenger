...
contract SimpleMessages {
    ...
    function addUserToChat(address _userAddress, uint256 _chatID) public {
        ...
        chats[_chatID].currentUsers.push(_userAddress); // add user
    ...
    function removeUserFromChat(address _userAddress, uint256 _chatID) public {
        ...
        assert(userIndex > -1);
        chats[_chatID].currentUsers[uint256(userIndex)] = chats[_chatID].currentUsers[chats[_chatID].currentUsers.length - 1];
        chats[_chatID].currentUsers.pop();
    }
    function findUser(address _userAddress, uint256 _chatID) public view returns(int256) {
        assert(chats[_chatID].currentUsers.length > 0);
        for (uint i = 0; i < chats[_chatID].currentUsers.length; i++) {
          // do something
    ...
    function getChatMessages(uint256 _chatID) view public returns(Message[] memory){
    ...
    /// @return Message array
    function getMessagesSlice(uint256 _chatID, uint256 start, uint256 end) view public returns (Message[] memory){
        assert(chats[_chatID].chatID != 0); // Chat exists
        assert(findUser(msg.sender, _chatID) > -1); // User is in chat
    ...
    function postMessage(uint256 _chatID, string memory _message, uint256 _unixTimestamp) public {
        Message memory _newMessage = Message(msg.sender, _message, block.timestamp, _unixTimestamp);
